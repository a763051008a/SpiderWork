...
class Login(Spider):
    ...
    def parse(self, response):
        ...
        formdata = {
            'password': 'chenghao',
            'phone_num': '17701689367',
            'email': '邮箱二选一, 不选的留空'
        }
        return FormRequest.from_response(
            url='https://passport.lagou.com/login/login.html?msg=validation&uStatus=2&clientIp=210.22.91.8/{}'.format('phone_num' if formdata.get('phone_num', None) else 'email'),
            method="POST",
            response=response,
            formxpath='//form[1]',
            formdata=formdata,
